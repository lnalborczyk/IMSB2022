...
probabilité d'observer la valeur de la statistique de test observée, ou
une valeur plus extrême, sous l'hypothèse nulle.
...
Le BF est un facteur qui nous indique de combien nos *prior odds*
doivent changer, au vue des données. Il ne nous dit pas quelle est
l'hypothèse la plus probable, sachant les données (sauf si les *prior
odds* sont 1:1).
...
On fait une expérience et on calcule un $BF_{10} = 27$. Quelle est la
probabilité a posteriori de H1 ?
...
-   À la probabilité *a priori* ou *a posteriori* d'un modèle (par
    rapport à un autre modèle), c'est à dire $\Pr(M_{i})$. Voir ce
    [blogpost](http://www.nicebread.de/two-meanings-of-priors-1/).

-   Aux prédictions du modèle, par exemple
    $\beta \sim \mathrm{Normal}(0, 10)$. Voir ce
...
sous-apprentissage et sur-apprentissage (*overffitting* et
*underfitting*). Comment s'en sortir ?
...
si on l'a réduite, ou pas)... S'il existe $n$ événements possibles, et
que chaque événement $i$ a pour probabilité $p_{i}$, alors une mesure de
...
probabilités est la log-probabilité moyenne d'un événement*.
...
Alors,
$H(p) = - (p_{1} \cdot \log(p_{1}) + p_{2} \cdot \log(p_{2})) \approx 0.61$.
...
pleuve ou qu'il y fasse beau est de $p_{1} = 0.01$ et $p_{2} = 0.99$.
...
Par exemple, supposons que la *véritable* distribution de nos événements
(soleil vs pluie) soit $p_{1}=0.3$ et $p_{2}=0.7$. Si nous pensons
plutôt que ces événements arrivent avec une probabilité $q_{1}=0.25$ et
...
OK, mais nous ne connaissons pas la distribution *target* (la réalité),
...
entre chaque modèle et notre distribution cible. On a donc seulement
...
$\mathbb{E} [\log(q_{i})] - \mathbb{E} [\log(r_{i})]$.
...
utiliser la
[déviance](https://en.wikipedia.org/wiki/Deviance_%28statistics%29) d'un
...
Les fréquentistes multiplient le log-score par $-2$ car la différence de
...
$$\text{lppd}(y, \Theta) = \sum_{i} \log \frac{1}{S} \sum_{s} p(y_{i} | \Theta_{s})$$
...
calculer la déviance du modèle sur cet échantillon ($D_{train}$ ou
$D_{in}$). Si nous acquérons ensuite un nouvel échantillon de taille $N$
...
d'entraînement (que nous appellerons $D_{test}$ ou $D_{out}$).
...
Une autre manière de lutter contre l'*overfitting* est d'utiliser des
priors *sceptiques* qui vont venir ralentir l'apprentissage réalisé sur
...
deux parties (*training* et *test*) afin de choisir le prior qui produit
...
NB : l'AIC fonctionne bien uniquement quand le nombre d'observations $N$
est largement supérieur au nombre de paramètres $p$. Dans le cas
contraire, on utilise plutôt l'AICc [@burnham2002; @burnham2004].

## Deviance information criterion

Une autre condition de l'AIC est que les priors soient plats (i.e., peu
informatifs) ou dépassés par la vraisemblance (on a beaucoup de
...
cette condition. Il est souvent plus précis que le DIC.
...
l'analogue point par point de la déviance, moyenné sur toute la
...
effectif, $p_{WAIC}$. On définit
$\text{var}_{\theta}\log[p(y_{i} | \theta)]$ comme la variance de la
log-vraisemblance pour chaque observation $i$ de l'échantillon
d'entraînement.

$$p_{\text{WAIC}} = \sum_{i} \text{var}_{\theta}\log[p(y_{i} | \theta)]$$
...
$$
\text{WAIC}(y, \Theta) = - 2 \big(\text{lppd} - \underbrace{\sum_{i} \text{var}_{\theta}\log[p(y_{i} | \theta) ]}_{\text{penalty term}}\big)
$$
...
Le WAIC est aussi un estimateur de la déviance *out-of-sample*. La
...
Nous allons essayer de prédire les kg par gramme de lait (`kcal.per.g`)
...
Cette fonction permet simplement de passer d'un WAIC à une probabilité
(softmax). Le modèle `mod2.4` a un poids de $0.96$, qui le place en tête
du jeu de modèles. N'oublions cependant pas que nous disposons seulement
de 17 observations...
...
-   Combiner ces valeurs simulées dans un *ensemble* de prédictions
    pondérées par le poids du modèle
...
équilibre entre sous-apprentissage (*under-fitting*) et
...
# sampling some rows of the data
