...
\mu_{j} &= \alpha + \alpha_{\text{study}[j]} \\
\alpha_{\text{study}[j]} &\sim \mathrm{Normal}(0, \tau_{s}) \\
\alpha &\sim \mathrm{Normal}(0, 1) \\
...
(qui est connue), on estime aussi la variabilité des effets entre les
...
\alpha_{\text{study}[j]} &\sim \mathrm{Normal}(0, \tau_{s}) \\
\alpha_{\text{experiment}[ij]} &\sim \mathrm{Normal}(0, \tau_{e}) \\
\alpha &\sim \mathrm{Normal}(0, 1) \\
...
# comparaison des WAIC de chaque modèle
# model_weights(mod5, mod6, mod7, mod8, weights = "waic") %>% round(digits = 3)
