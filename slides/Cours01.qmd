...
Les slides, données, et scripts seront disponibles juste avant chaque séance sur Github : <https://github.com/lnalborczyk/IMSB2022>.
...
```{r frequency, fig.width = 7.5, fig.height = 5, `code-line-numbers` = "|5"}
library(tidyverse)
...
##  {background-image="figures/pill2.jpg"}
...
RNGkind() # default random number generator
```

## Mersenne-Twister

Inventé par Makoto Matsumoto et Takuji Nishimura en 1997, le [Mersenne Twister](https://en.wikipedia.org/wiki/Mersenne_Twister) est un générateur de nombres pseudo-aléatoires. L'algorithme Mersenne-Twister est le générateur par défaut en `Python`, `Ruby`, `R`, `Matlab`...

![](figures/mersenne.png){fig-align="center" width="50%"}
...
![](figures/penguin.jpg){fig-align="center" width="50%"}
...
::: columns
::: {.column width="50%"}
<center>Hypothèse : :large_blue_circle: :large_blue_circle: :white_circle: :white_circle:</center>
:::
...
-   $\color{orangered}{p(x\vert \theta)}$ [la probabilité conditionnelle des données $x$ sachant le paramètre $\theta$ $\color{orangered}{[p(x | \theta = \theta)]}$.]{style="color:orangered"}
-   $\color{orangered}{p(x\vert \theta)}$ [une fois que la valeur de $x$ est connue, est vue comme la fonction de vraisemblance (*likelihood*) du paramètre $\theta$. Attention, il ne s'agit pas d'une distribution de probabilité valide $\color{orangered}{[p(x = x | \theta)]}$.]{style="color:orangered"}
-   $\color{steelblue}{p(\theta)}$ [la probabilité a priori de $\theta$.]{style="color:steelblue"}
-   $\color{purple}{p(\theta \vert x)}$ [la probabilité a posteriori de $\theta$ (sachant $x$).]{style="color:purple"}
...
On remarque dans le cas précédent que $p(\text{cheveux = blonds} \ |\ \text{yeux = bleus})$ **n'est pas nécessairement égal** à $p(\text{yeux = bleus} \ |\ \text{cheveux = blonds})$.

. . .

<iframe align="left" border="0" frameborder="0" height="500" width="600" src="https://twitframe.com/show?url=https://twitter.com/BillGates/status/1118196606975787008">

</iframe>

<br>

Autre exemple : la probabilité de mourir sachant qu'on a été attaqué par un requin n'est pas la même que la probabilité d'avoir été attaqué par un requin, sachant qu'on est mort ([confusion of the inverse](https://en.wikipedia.org/wiki/Confusion_of_the_inverse)). De la même manière, $p(\text{data} | \mathcal{H}_{0}) \neq p(\mathcal{H}_{0} | \text{data})$.
...
$\color{orangered}{p(x\vert \theta)}$ [représente la probabilité conditionnelle des données $x$ sachant le paramètre $\theta$, qu'on appelle aussi la *likelihood* (*ou fonction de vraisemblance*) du paramètre $\theta$.]{style="color:orangered"}
...
[$p(x)$ la probabilité marginale de $x$ (sur $\theta$). Sert à normaliser la distribution.]{style="color:green"}
...
$\color{purple}{p(\theta \vert x)}$ [la probabilité a posteriori de $\theta$ sachant $x$, c'est à dire ce qu'on sait de $\theta$ après avoir pris connaissance de $x$.]{style="color:purple"}
...
$\Pr(\text{P3} | \text{V1}) = \dfrac{1}{2}, \quad \Pr(\text{P3} | \text{V2}) = 1, \quad \Pr(\text{P3} | \text{V3}) = 0.$
...
$\Pr(\text{V1} | \text{P3}) = \dfrac{\Pr(\text{P3} | \text{V1}) \times \Pr(\text{V1})}{\Pr(\text{P3})} = \dfrac{\dfrac{1}{2} \times \dfrac{1}{3}}{\dfrac{1}{2}} = \dfrac{1}{3}$

. . .

$\Pr(\text{V2} | \text{P3}) = \dfrac{\Pr(\text{P3} | \text{V2}) \times \Pr(\text{V2})}{\Pr(\text{P3})} = \dfrac{1 \times \dfrac{1}{3}}{\dfrac{1}{2}} = \dfrac{2}{3}$
