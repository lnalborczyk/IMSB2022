...
On remarque dans le cas précédent que $\Pr(\text{cheveux = blonds} | \text{yeux = bleus})$ **n'est pas nécessairement égal** à $\Pr(\text{yeux = bleus} | \text{cheveux = blonds})$.
...
-   Quand on le considère comme fonction de $\theta$, on parle de **vraisemblance** (likelihood) : $L(\theta|x) = p(X = x|\theta)$.
...
$\color{orangered}{p(x\vert \theta)}$ [représente la probabilité conditionnelle des données $x$ sachant le paramètre $\theta$, qu'on appelle aussi la fonction de vraisemblance (likelihood function) du paramètre $\theta$.]{style="color:orangered"}
