...
$$
\begin{align}
w_{i} &\sim \mathrm{Normal}(\mu_{i}, \sigma) \\
\mu_{i} &= \alpha \\
\alpha &\sim \mathrm{Normal}(5, 10) \\
...
$$
p(x\ |\ x_{0}, \gamma) = \Big(\pi \gamma \Big[1 + \big(\tfrac{x-x_{0}}{\gamma}\big)^{2}\Big] \Big)^{-1}
$$
...
\begin{align}
w_{i} &\sim \mathrm{Normal}(\mu_{i}, \sigma) \\
\mu_{i} &= \alpha_{\text{café}[i]} \\
\alpha_{\text{café}[i]} &\sim \mathrm{Normal}(5, 10) \\
\sigma &\sim \mathrm{HalfCauchy}(0, 2) \\
\end{align}
$$
...
$$
\begin{align}
\text{Niveau 1}: w_{i} &\sim \mathrm{Normal}(\mu_{i}, \sigma) \\
\mu_{i} &= \alpha_{\text{café}[i]} \\
\text{Niveau 2}: \alpha_{\text{café}} &\sim \mathrm{Normal}(\alpha,\sigma_{\text{café}}) \\
\alpha &\sim \mathrm{Normal}(5, 10) \\
\sigma_{\text{café}} &\sim \mathrm{HalfCauchy}(0, 2) \\
\sigma &\sim \mathrm{HalfCauchy}(0, 2) \\
\end{align}
$$
...
w_{i} &\sim \mathrm{Normal}(\mu_{i}, \sigma) \\
\mu_{i} &= \alpha_{\text{café}[i]} \\
\alpha_{\text{café}} &\sim \mathrm{Normal}(\alpha, \sigma_{\text{café}}) \\
\end{align}
$$
...
\begin{align}
w_{i} &\sim \mathrm{Normal}(\mu_{i}, \sigma) \\
\mu_{i} &= \alpha + \alpha_{\text{café}[i]} \\
\alpha_{\text{café}} &\sim \mathrm{Normal}(0, \sigma_{\text{café}}) \\
\alpha &\sim \mathrm{Normal}(5, 10) \\
\end{align}
$$
...
where $\bar{y}$ is the grand average, $y$ an individual estimation and
$c$ a constant, the "shrinking factor" [@efron1977].

## Shrinkage magic [@efron1977]

The James-Stein estimator is determined both by the variability in the
measure (e.g., its standard deviation, influencing the shrinking factor
$c$) and by its distance to the average estimation (i.e.,
$y - \bar{y}$). In other words, extreme observations are trusted less.
Shrinkage therefore acts as **a safeguard against overfitting** in
multilevel models.
...
La stratégie *complete pooling* en général *underfit* les données
...
aussi). La stratégie *partial pooling* (celle des modèles multi-niveaux)
permet de balancer *underfitting* et *overfitting*.
...
On compare le premier modèle (*complete pooling model*) et le troisième
...
mais le modèle 3 est plus incertain de sa prédiction que le modèle 1...
...
effects* et *varying effects*, et de toujours utiliser la modèlisation
...
classe dans une école dans une ville...). Voir [ce
thread](https://stats.stackexchange.com/questions/228800/crossed-vs-nested-random-effects-how-do-they-differ-and-how-are-they-specified)
...
\begin{align}
w_{i} &\sim \mathrm{Normal}(\mu_{i}, \sigma) \\
\mu_{i} &= \alpha + \alpha_{\text{café}[i]} + \alpha_{\text{âge}[i]} \\
\alpha_{\text{café}} &\sim \mathrm{Normal}(5, \sigma_{\text{café}}) \\
\alpha_{\text{âge}} &\sim \mathrm{Normal}(5, \sigma_{\text{âge}}) \\
\alpha &\sim \mathrm{Normal}(0, 10) \\
\sigma_{\text{café}} &\sim \mathrm{HalfCauchy}(0, 2) \\
\sigma_{\text{âge}} &\sim \mathrm{HalfCauchy}(0, 2) \\
\sigma &\sim \mathrm{HalfCauchy}(0, 2) \\
\end{align}
$$
...
\begin{align}
w_{i} &\sim \mathrm{Normal}(\mu_{i},\sigma) \\
\mu_{i} &= \alpha_{\text{café}[i]} + \beta_{\text{café}[i]} A_{i}  \\
\end{align}
$$

Où $A_{i}$ est une *dummy variable* codée 0/1 pour le matin et
l'après-midi et $\beta_{\text{café}}$ est un paramètre de différence
entre le matin et l'après-midi.

. . .

Remarque : on sait que les cafés ont des intercepts et des pentes qui
covarient... Les cafés populaires seront surchargés le matin et beaucoup
...
\sigma_{\alpha} &\sim \mathrm{HalfCauchy} (0, 1) \\
\sigma_{\beta} &\sim \mathrm{HalfCauchy} (0, 1) \\
\textbf{R} &\sim \mathrm{LKJ}(2) \\
\end{align}
$$
...
D'après @lewandowski2009. Un seul paramètre $\zeta$ spécifie la
concentration de la distribution du coefficient de corrélation. Le prior
$\mathrm{LKJ}(2)$ définit un prior peu informatif pour $\rho$ qui est
sceptique des corrélations extrêmes (i.e., proches de $-1$ ou $1$).
...
modèle (*partial pooling model*), et le dernier modèle.
