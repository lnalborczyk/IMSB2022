...
Assume a model with a Normal likelihood function for the data $y$, a Normal prior on the mean $\mu$, and an Exponential prior on the standard deviation $\sigma$.

$$
\begin{aligned}
\color{orangered}{y_{i}} \ &\color{orangered}{\sim \mathrm{Normal}(\mu, \sigma)} \\
\color{steelblue}{\mu} \ &\color{steelblue}{\sim \mathrm{Normal}(100, 10)} \\
\color{steelblue}{\sigma} \ &\color{steelblue}{\sim \mathrm{Exponential}(0.1)} \\
\end{aligned}
$$
## Rappels : prior predictive checking

On peut simuler des données à partir de ce modèle, sachant ces priors.
...
nombre de points dans le disque au nombre de tirages $\frac{N_{\text{inner}}}{N_{\text{total}}}$, on obtient donc une approximation de $\pi / 4$.
