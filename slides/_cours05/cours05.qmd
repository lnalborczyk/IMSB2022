...
#| echo: false
#| out.width: "100%"
knitr::include_url(url = "plotly1.html")
```
...
-   La distribution a priori est un *prior conjugué* de la fonction de
    ...
    une solution analytique (qu'on peut calculer de manière exacte) pour
...
$\longrightarrow~$ Présenter les algorithmes classiques
...
-   **Markov** chain Monte Carlo <br> $\longrightarrow~$ La valeur de
    ...
    $\longrightarrow~P(\theta^{t+1}~|~\theta^{t},\theta^{t-1}, \dots,\theta^{1})~=~P(\theta^{t+1}~|~\theta^{t})$
...
$x^{2} + y^{2} \leqslant 1$. La probabilité que le point $M$ appartienne
au disque est $\pi / 4$ puisque le quart de disque est de surface
$\sigma = \frac{\pi R^{2}}{4} = \frac{\pi}{4}$, et le carré qui le
contient est de surface $S = R^{2} = 1$. Si la loi de probabilité du
tirage de point est uniforme, la probabilité de tomber dans le quart de
disque vaut $\frac{\sigma}{S} = \frac{\pi}{4}$. En faisant le rapport du
nombre de points dans le disque au nombre de tirages, on obtient une
approximation du nombre $\pi / 4$ si le nombre de tirages est grand.
...
le maximum d'une fonction (optimisation) via **simulated annealing** ou
**recuit simulé**
...
la fonction de vraisemblance $p(\text{data} | \theta_{1}, \theta_{2})$
<br>

. . .

Mais on ne sait pas calculer la distribution postérieure...
$p(\theta_{1}, \theta_{2} | \text{data}) = \dfrac{p(\text{data} | \theta_{1}, \theta_{2}) p(\theta_{1}) p(\theta_{1})}{p(\text{data})}$
<br>
...
#| echo: false
#| out.width: "100%"
knitr::include_url(url = "plotly2.html")
```

## Méthodes Monte Carlo : Exemple

Considérons un exemple simple : Soit un paramètre $\theta$ avec 7
valeurs possibles et la fonction de répartition suivante.
...
Estimation de cette distribution par tirage aléatoire : Cela revient à
...
$~\bullet~$ La distribution des échantillons obtenus converge vers la
"vraie" distribution <br> $~\bullet~$ Mais, nécessite généralement
beaucoup d'échantillons (ici pour 7 valeurs de paramètres et 100
tirages) <br> $~\bullet~$ Aucun contrôle sur la vitesse de
convergence... <br> $~\bullet~$ Et si on abandonnait l'échantillonnage
indépendant ? <br>
...
$\bullet~$ On fait une proposition de déplacement sur la base de la
valeur courante du paramètre <br> $\bullet~$ On réalise un tirage
aléatoire pour accepter ou rejeter la nouvelle position <br>

. . .

### Deux idées centrales

$\bullet~$ La proposition doit favoriser les valeurs de paramètre les
plus probables <br> $~~\longrightarrow~$ On parcourt plus souvent ces
valeurs de paramètres <br>

. . .

$\bullet~$ La proposition doit se limiter aux valeurs adjacentes au
paramètre courant <br> $~~\longrightarrow~$ On augmente la vitesse de
...
Puis, accepter ou rejeter la proposition de déplacement. On compare le
ratio de probabilités $\Pr_{\text{move}}$ à un tirage uniforme. On
génère une valeur $u$ suivant $u \sim \mathrm{Uniform(0, 1)}$. Ensuite :

$$
\begin{aligned}
\text{Si }& u \leq \Pr_{\text{move}} \text{, Alors on accepte le déplacement}\\
\text{Si }& u > \Pr_{\text{move}} \text{, Alors on rejete le déplacement}\\
\end{aligned}
$$
...
# barplot
barplot(
  table(trajectory[idxToPlot]), col = prior_color,
  horiz = TRUE, axes = FALSE, axisnames = FALSE
  )
```
...
$\color{orangered}{P(z~|~\theta,N)=\theta^z~(1-\theta)^{(N-z)}}$ <br>
$~\bullet~$ Le prior est donné par :
$\color{steelblue}{P(\theta~|~a,b) \propto \theta^{(a-1)}(1-\theta)^{(b-1)}}$
...
Le déplacement est modélisé par une distribution normale :
...
prior :
$\color{orangered}{\theta^z~(1-\theta)^{(N-z)}}\color{steelblue}{\theta^{(a-1)}(1-\theta)^{(b-1)}}$<br>
...
Deux indices permettent d'évaluer la qualité de l'échantillonnage : <br>
...
```{r metropolis-hastings1, eval = TRUE, echo = TRUE}
target <- function(x) {
...
```{r metropolis-hastings2, eval = TRUE, echo = TRUE, fig.width = 10, fig.height = 5}
...
```{r metropolis-hastings3, eval = TRUE, echo = TRUE, fig.width = 10, fig.height = 5}
data.frame(z1 = z1, z2 = z2) %>%
  pivot_longer(cols = z1:z2) %>%
  rownames_to_column() %>%
...
Les algorithmes Metropolis-Hastings et Gibbs ont de mauvaises
performances lorsque les paramètres du modèle sont fortement corrêlés.
L'algorithme **Hamiltonian Monte Carlo** repose sur l'algorithme
Metropolis mais résout ces problème en utilisant la géométrie de
l'espace postérieur. On utilise l'opérateur hamiltonien (*hamiltonians*)
...
$$U(\theta) = -\log[p(data | \theta) \times p(\theta)]$$
...
Si on "suit" simplement la gravité de notre position dans l'espace
postérieur (imaginons que nous suivions la trajectoire d'une luge ou
d'une bille dans cet espace), on arrivera à un minimum de cet espace,
mais on veut explorer tout l'espace proportionnellement à la densité
postérieure, pas seulement les endroits de forte densité...

. . .

L'algorithme Metropolis repose sur les principes suivants :

$\longrightarrow~$ La proposition de déplacement est toujours une
gaussienne multivariée centrée sur la position courante.

. . .

$\longrightarrow~$ La proposition de déplacement est une gaussienne qui
garde toujours la même forme (variance constante).

. . .

L'algorithme HMC repose sur une autre idée : Adapter la proposition de
déplacement en fonction de la position courante du paramètre et de la
géométrie du postérieur aux alentours de cette position.
...
$\longrightarrow~$ Sélectionner un point de départ $\theta_{0}$ : On
peut sélectionner n'importe quelle valeur de $\theta$ dans l'espace des
paramètres

. . .

$\longrightarrow~$ Générer une nouvelle position à partir de la fonction
de potentiel <br> $~\bullet~$ *L'algorithme génère une proposition par
analogie au lancer d'une bille sur la fonction de potentiel* <br>
$~\bullet~$ *Le pas de déplacement, ainsi que le nombre de déplacement,
sont fixés à l'avance* <br> $~\bullet~$ *La force avec laquelle on lance
la bille est déterminée par une loi normale multivariée :*
$m \sim \mathrm{MVNormal}(\mu, \Sigma)$ <br> $~\bullet~$ *Estimation de
la trajectoire discrétisée via la méthode leapfrog* <br>

. . .

$\longrightarrow~$ Accepter ou rejeter la proposition de déplacement
suivant la probabilité (où $\phi$ est le moment associé à la bille) :

$$
\Pr_{déplacement} = \min \left(\frac{\Pr(\theta_{proposé}~|~D)~\Pr(\phi_{final})}{\Pr(\theta_{courant}~|~D)~\Pr(\phi_{initial})}, 1 \right)
$$

$\longrightarrow~$ On enregistre la nouvelle position et on
recommence...
...
-   L'algorithme HMC est particulièrement utile lorsque le modèle
    contient un grand nombre de paramètres et quand les paramètres sont
    corrélés entre eux.

-   Il nécessite un paramétrage assez fin. Des logiciels proposent des
    méthodes clés en main très efficaces. <br>

-   Le package brms utilise une variation de cet algorithme
    (l'algorithme NUTS) qui permet d'éviter les trajectoires cycliques.
...
la proposition pour Gibbs, ou variance du moment initial pour HMC).
...
-   Les valeurs de la chaîne doivent être représentatives de la
    distribution postérieure <br> $~\bullet~$ Ces valeurs ne doivent pas
    dépendre du point de départ <br> $~\bullet~$ Ces valeurs ne doivent
...
-   La chaîne doit être suffisamment longue pour assurer la précision et
    la stabilité du résultat <br> $~\bullet~$ La tendance centrale et le
    HDI calculés à partir de la chaîne ne doivent pas changer si on
    relance la procédure <br>

-   La chaîne doit être générée de manière efficace (avec le moins
...
L'erreur standard d'un ensemble d'échantillons est donné :
...
(*samplers*) pour obtenir des échantillons issus de la distribution
postérieure (non-normalisée). Ces échantillons peuvent ensuite être
utilisés pour calculer différentes statistiques sur la distribution
...
très lente... L'algorithme Gibbs accélère la convergence mais nécessite
qu'on l'on puisse calculer la distribution conditionnelle des
paramètres. De plus, cet algorithme ne fonctionne pas bien lorsqu'il
...
beaucoup plus rapidement que les deux précédents et moins d'échantillons
...
estimations de ce modèle (interprétation, diagnostiques des MCMCs).

$$
\begin{aligned}
\log(\text{gdp}_{i}) &\sim \mathrm{Normal}(\mu_{i}, \sigma_{i}) \\
\mu_{i} &= \alpha + \beta_{1} \cdot \text{rugged}_{i} + \beta_{2} \cdot \text{continent}_{i} + \beta_{3} \cdot (\text{rugged}_{i} \cdot \text{continent}_{i}) \\
\alpha &\sim \cdots \\
\beta_{1}, \beta_{2}, \beta_{3} &\sim \cdots \\
\end{aligned}
$$
...
```{r mod2-pairs, eval = TRUE, echo = TRUE, fig.width = 10, fig.height = 7.5}
pairs(mod2)
```
