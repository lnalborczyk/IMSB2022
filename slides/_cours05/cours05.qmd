...
$p(\theta_{1}, \theta_{2} \given \text{data}) = \dfrac{p(\text{data} \given \theta_{1}, \theta_{2}) p(\theta_{1}) p(\theta_{1})}{p(\text{data})}$
<br>
...
theta <- 1:7
ptheta <- theta
samples <- sample(x = theta, prob = ptheta, size = niter, replace = TRUE)
```
...
- Mais, nécessite généralement beaucoup d'échantillons...
...
\text{Si }& u > \Pr_{\text{move}} \text{, Alors on rejette le déplacement}\\
\end{aligned}
$$
...
metropolis <- function (niter = 1e2, startval = 3) {
    ...
    for (i in 2:niter) {
        ...
        # x[i] <- ifelse(runif(1) < prob_move, proposal, currentPosition)
...
$\color{orangered}{p(y \given \theta, n) = \theta^y(1 - \theta)^{(n - y)}}$ <br>
...
z1 <- metropolis_hastings(niter = 1000, startval = 0.5)
z2 <- metropolis_hastings(niter = 1000, startval = 0.5)
...
$$U(\theta) = -\log[p(data \given \theta) \times p(\theta)]$$
