...
L’objectif de cette formation est de vous faire découvrir l’approche bayésienne. Les concepts et outils qui seront présentés tout au long de la formation seront illustrés par des cas concrets d’analyse de données. Ce cours est construit autour du langage `R` et du paquet `brms`, une interface au langage probabiliste `Stan`. À la fin du cours, vous devriez pouvoir construire et fitter des modèles de régressions adapté à votre problème avec `brms`.
...
Au cours de cette formation, nous privilégierons une approache... il y aura peu d'équations, et plus de code, pour vous permettre de comprendre les concepts mobilisés par la pratique. Il y aura un peu moins de :

$$
f(x) = \dfrac{1}{\sqrt{2\pi}} e^{-\frac12 x^2}
$$

Et un peu plus de code en `R` :
